...
*/
import { AVAILABLE_KIND, EFFECT_KIND, EFFECT_SPECIAL_ACTION_KIND, Interpreter, ITEM_KIND, SONG_KIND, TARGET_KIND, Utils, } from '../Common/index.js';
...
class CommonSkillItem extends Icon {
    ...
    getEffects() {
        ...
        for (const effect of this.effects) {
            if (effect.kind === EFFECT_KIND.PERFORM_SKILL) {
                const skill = Data.Skills.get(effect.performSkillID.getValue());
    ...
    isPossible(target, checkCost = true) {
        ...
        if (this.effects.some((effect) => {
            return (effect.kind === EFFECT_KIND.SPECIAL_ACTIONS &&
                effect.specialActionKind === EFFECT_SPECIAL_ACTION_KIND.APPLY_WEAPONS);
        })) {
            if (!Scene.Map.current.user.player.equip.some((item) => item === null ||
