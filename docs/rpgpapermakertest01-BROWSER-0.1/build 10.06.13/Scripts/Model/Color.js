...
export class Color extends Base {
    ...
    getHex(tone) {
        let hex;
        if (tone) {
            const rgb = new THREE.Vector3(Math.max(Math.min(this.color.r + tone.x, 1), -1), Math.max(Math.min(this.color.g + tone.y, 1), -1), Math.max(Math.min(this.color.b + tone.z, 1), -1));
...
Color.GREEN = Color.createColor(25, 214, 25);
Color.RED = Color.createColor(216, 33, 17);
Color.WHITE = Color.createColor(255, 255, 255);
Color.BLACK = Color.createColor(0, 0, 0);
Color.GREY = Color.createColor(150, 150, 150);
