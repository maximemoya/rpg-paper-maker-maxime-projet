...
export class Effect extends Base {
     ...
     */
    isAnimated() {
        return (this.kind === EFFECT_KIND.DAMAGES ||
        ...
        switch (this.kind) {
            case EFFECT_KIND.DAMAGES: {
                ...
                for (const battler of targets) {
                    ...
                    if (this.isDamagePrecision) {
                        const precision = Interpreter.evaluate(this.damagePrecisionFormula.getValue(), {
                            user,
                            target,
                        });
                    ...
                    if (damage !== null) {
                        if (this.isDamageCritical) {
                            const critical = Interpreter.evaluate(this.damageCriticalFormula.getValue(), {
                                user,
                                target,
                            });
            ...
            case EFFECT_KIND.STATUS: {
                for (const battler of targets) {
                    const target = battler.player;
                    let miss = false;
                    let precision = Interpreter.evaluate(this.statusPrecisionFormula.getValue(), {
                        user,
                        target,
                    });
                    const id = this.statusID.getValue();
                    // Handle resistance
        ...
        for (const battler of Scene.Map.current.targets) {
            ...
            if (damage !== null) {
                ...
                if (this.isDamageVariance) {
                    const variance = Math.round((damage *
                ...
                if (this.isDamageElement) {
                    const element = this.damageElementID.getValue();
                    ...
                    for (const targetElement of target.elements) {
                        const efficiency = modelElement.efficiency.get(targetElement.getValue());
                        damage *= efficiency ? efficiency.getValue() : 1;
                    }
                    const fixRes = target[Data.BattleSystems.getStatistic(Data.BattleSystems.getStatisticElement(element))
                        .abbreviation];
                    const percentRes = target[Data.BattleSystems.getStatistic(Data.BattleSystems.getStatisticElementPercent(element))
                        .abbreviation];
                ...
                if (this.isDamageCritical) {
                    ...
                    if (battler.tempIsDamagesCritical ||
                        (battler.tempIsDamagesCritical === null && Mathf.randomPercentTest(critical))) {
                        damage = Interpreter.evaluate(Data.BattleSystems.formulaCrit.getValue(), {
                            user,
                            target,
                            damage,
                        });
                ...
                if (this.isDamagesMinimum) {
                    damage = Math.max(damage, Interpreter.evaluate(this.damagesMinimumFormula.getValue(), {
                        user,
                        target,
                    }));
                }
                if (this.isDamagesMaximum) {
                    damage = Math.min(damage, Interpreter.evaluate(this.damagesMaximumFormula.getValue(), {
                        user,
                        target,
                    }));
                }
            ...
            if (this.isDamageDisplayName) {
                switch (this.damageKind) {
                    case DAMAGES_KIND.STAT:
                        damageName = Data.BattleSystems.getStatistic(this.damageStatisticID.getValue()).name();
                        break;
                    case DAMAGES_KIND.CURRENCY:
                        damageName = Data.Systems.getCurrency(this.damageCurrencyID.getValue()).name();
                        break;
                    default:
                        break;
                }
            ...
            switch (this.damageKind) {
                case DAMAGES_KIND.STAT: {
                    const stat = Data.BattleSystems.getStatistic(this.damageStatisticID.getValue());
                ...
                case DAMAGES_KIND.CURRENCY: {
                    const currencyID = this.damageCurrencyID.getValue();
                ...
                case DAMAGES_KIND.VARIABLE: {
                    ...
                    Game.current.variables.set(this.damageVariableID, Game.current.getVariable(this.damageVariableID) - damage);
    ...
    executeStatus(user) {
        ...
        for (const battler of Scene.Map.current.targets) {
            ...
            let miss = true;
            if (battler.tempIsDamagesMiss === false ||
                (battler.tempIsDamagesMiss === null && Mathf.randomPercentTest(precision))) {
                miss = false;
    ...
    toStringDamages(user, target) {
        ...
        return `${damage > 0 ? Data.Languages.extras.damage.name() : Data.Languages.extras.heal.name()} ${damageName}: ${min === max ? min : min + ' - ' + max}${options.length > 0 ? ` [${options.join(' - ')}]` : ''}`;
    }
    /**
     ...
     */
    toStringStatus(user, target) {
        return `${this.isAddStatus ? Data.Languages.extras.add.name() : Data.Languages.extras.remove.name()} ${Data.Status.get(this.statusID.getValue()).name()} [${Data.Languages.extras.precision.name()}: ${Interpreter.evaluate(this.statusPrecisionFormula.getValue(), { user, target })}%]`;
    }
    /**
     ...
     */
    toStringAddRemoveSkill() {
        return `${this.isAddSkill ? Data.Languages.extras.add.name() : Data.Languages.extras.remove.name()} ${Data.Languages.extras.skill.name()} ${Data.Skills.get(this.addSkillID.getValue()).name()}`;
    }
    /**
     ...
     */
    toStringPerformSkill() {
        return `${Data.Languages.extras.performSkill.name()} ${Data.Skills.get(this.performSkillID.getValue()).name()}`;
    }
    /**
        ...
        switch (this.kind) {
            ...
            case EFFECT_KIND.COMMON_REACTION:
                this.commonReaction =
                    json.cr === undefined
                        ? null
                        : Manager.Events.getEventCommand(json.cr);
                break;
            case EFFECT_KIND.SPECIAL_ACTIONS:
