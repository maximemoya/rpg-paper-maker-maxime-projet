...
import { ALIGN, BATTLE_STEP, CHARACTER_KIND, Constants, EFFECT_SPECIAL_ACTION_KIND, Interpreter, MAP_TRANSITION_KIND, Platform, ScreenResolution, SONG_KIND, } from '../Common/index.js';
import { Battler, Game, Player, Position, WindowBox, WindowChoices } from '../Core/index.js';
// -------------------------------------------------------
//
//  CLASS Battle
//
    ...
    initializeAlliesBattlers() {
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (!(center instanceof THREE.Vector3)) {
                Platform.showErrorMessage('Heroes battlers center offset incorrect return, should be a THREE.Vecto3: ' + center);
            }
            ...
            if (!(offset instanceof THREE.Vector3)) {
                Platform.showErrorMessage('Heroes battlers offset incorrect return, should be a THREE.Vecto3: ' + center);
            }
    ...
    initializeEnemiesBattlers() {
        ...
        for (let i = 0; i < l; i++) {
            ...
            if (troopMonster.isSpecificPosition) {
                ...
                center = Interpreter.evaluate(Data.BattleSystems.troopsBattlersCenterOffset.getValue());
                if (!(center instanceof THREE.Vector3)) {
                    Platform.showErrorMessage('Troops battlers center offset incorrect return: ' + center);
                }
                offset = Interpreter.evaluate(Data.BattleSystems.troopsBattlersOffset.getValue(), {
                    additionalName: 'i',
                    additionalValue: i,
                });
                if (!(offset instanceof THREE.Vector3)) {
