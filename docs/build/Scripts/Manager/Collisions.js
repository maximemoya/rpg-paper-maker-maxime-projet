...
class Collisions {
    ...
    static createBox(detection = false) {
        const box = new THREE.Mesh(CustomGeometry.createBox(1, 1, 1), detection ? this.BB_MATERIAL_DETECTION : this.BB_MATERIAL);
     ...
     *  @returns {boolean}
     */
    static check(mapPortion, jpositionBefore, jpositionAfter, positionAfter, object, direction) {
     ...
     *  @returns {boolean}
     */
    static checkLands(mapPortion, jpositionBefore, jpositionAfter, object, direction) {
        ...
        if (lands !== null) {
            ...
            for (let i = 0, l = lands.length; i < l; i++) {
                ...
                if (objCollision !== null) {
                    ...
                    if (this.checkIntersectionLand(collision, boundingBox, object) ||
     ...
     *  @returns {boolean}
     */
    static checkSprites(mapPortion, jpositionAfter, object) {
     ...
     *  @returns {boolean}
     */
    static checkIntersectionSprite(boundingBox, fix, object, center) {
        if (boundingBox === null) {
            return false;
        }
        if (fix) {
     ...
     *  @returns {boolean}
     */
    static checkObjects3D(mapPortion, jpositionAfter, positionAfter, object) {
     ...
     *  @returns {boolean}
     */
    static checkMountains(mapPortion, jpositionAfter, positionAfter, object) {
        let yMountain = null;
        ...
        for (const mountain of mountains) {
            const result = this.checkMountain(mapPortion, jpositionAfter, positionAfter, object, mountain, yMountain, block);
            if (result[0]) {
     ...
     */
    static checkMountain(mapPortion, jpositionAfter, positionAfter, object, objCollision, yMountain, block) {
        ...
        if (w === 0) {
            ...
            if (objCollision.left && !mountain.left) {
                ...
                if (objCollision.top && !mountain.top) {
                    ...
                    if (Mathf.isPointOnRectangle(point, x + Data.Systems.SQUARE_SIZE, x + Data.Systems.SQUARE_SIZE + w, z, z + Data.Systems.SQUARE_SIZE)) {
                ...
                if (objCollision.top && !mountain.top) {
                    ...
                    if (Mathf.isPointOnRectangle(point, x, x + Data.Systems.SQUARE_SIZE, z + Data.Systems.SQUARE_SIZE, z + Data.Systems.SQUARE_SIZE + w)) {
            ...
            if (forceAlways ||
                (!forceNever && mountain.angle > Data.Systems.mountainCollisionAngle.getValue())) {
    ...
    static checkObjects(mapPortion, object) {
        ...
        return (this.checkObjectsList(mapPortion.objectsList, object) ||
